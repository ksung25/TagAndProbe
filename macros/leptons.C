...
//  ifs.close();
  ...
  if(do_electrons) {
    ...
    //electron_pair_tree->Branch("deltaR_probe_jet",      &deltaR_probe_jet,      "deltaR_probe_jet/F"   );  
  ...
  if(do_muons) {
    ...
    //muon_pair_tree->Branch("deltaR_probe_jet",      &deltaR_probe_jet,      "deltaR_probe_jet/F"   );  
    if(!real_data) {
  ...
  //TFile *puFile = TFile::Open("~/cms/cmssw/044/CMSSW_8_0_7_patch3/src/MitAnalysisRunII/data/80x/puWeights_80x.root","READ");
  ...
  for(unsigned int iif=0; iif<input_file_name_.size(); iif++) {
    ...
    for (Long64_t i=0; i<nentries;i++) {
      ...
      if(i % 100000==0) printf("\tEvent %lld / %lld\n", i, nentries);
      ...
      //  continue;
      //}
      //if(real_data && runNum > 274443) continue; //June 16th kluge
      if(real_data && runNum > max_runNum) max_runNum=runNum;
      if(real_data && runNum < min_runNum) min_runNum=runNum;
      //reweighting
      if(!real_data) {
        if(npv<=37) scale1fb = puWeights->GetBinContent(npv) * mcWeight / TMath::Abs(mcWeight);
        else scale1fb=0;
      ...
      for(int nj=0; nj<jetP4->GetEntriesFast(); nj++){
        ...
        if(((TLorentzVector*)(jetP4->At(nj)))->Pt() < 100) continue;
...
void make_trigger_soup_skim(
  ...
  string output_basename,
  int tag_trigger=0,
  string list_of_1l_soup_triggers="1", // comma delimited list of trigger bits "1,2,3"
  string list_of_2l_soup_triggers="1", // comma delimited list of trigger bits "1,2,3"
  bool verbose = false,
  ...
  std::vector<std::string> soup_triggers_1l_ = split(list_of_1l_soup_triggers, ',');
  std::vector<std::string> soup_triggers_2l_ = split(list_of_2l_soup_triggers, ',');
  ...
  //string output_dir = "/local/dhsu/nero/";
  ...
  for(unsigned int iif=0; iif<input_file_name_.size(); iif++) {
    ...
    if(first_file) {
      ...
      for(unsigned int it=0; it<soup_triggers_1l_.size(); it++) {
        int soup_trigger;
        std::istringstream(soup_triggers_1l_[it]) >> soup_trigger;
        descriptor_file << "\t" << trigger_names_[soup_trigger] << "\n";
      }
      for(unsigned int it=0; it<soup_triggers_2l_.size(); it++) {
        int soup_trigger;
        std::istringstream(soup_triggers_2l_[it]) >> soup_trigger;
        descriptor_file << "\t" << trigger_names_[soup_trigger] << "\n";
      }
    ...
    for (Long64_t i=0; i<nentries;i++) {
      ...
      std::vector<TLorentzVector> p4_soup_tag_, p4_soup_probe_candidate_;
      std::vector<int> q_soup_tag_, q_soup_probe_candidate_;
      std::vector<int> index_soup_tag_, index_soup_probe_candidate_;
      ...
      if(n_lep != 0 ) { for(unsigned int il=0; il<n_lep; il++) {
        ...
        if(is_probe_candidate) {
          p4_soup_probe_candidate_.push_back((*P4));
          q_soup_probe_candidate_.push_back(charge);
          index_soup_probe_candidate_.push_back(il);
        }
        if(is_tag) {
          p4_soup_tag_.push_back((*P4));
          q_soup_tag_.push_back(charge);
          index_soup_tag_.push_back(il);
        }
      }}
      ...
      for(unsigned int iTag=0; iTag < p4_soup_tag_.size(); iTag++) {
        // passing probes
        for(unsigned int iProbe=0; iProbe < p4_soup_probe_candidate_.size(); iProbe++) {
          *p4_tag     = p4_soup_tag_[iTag];
          *p4_probe   = p4_soup_probe_candidate_[iProbe];
          qtag    = q_soup_tag_[iTag];
          qprobe  = q_soup_probe_candidate_[iProbe];
          // make sure they aren't the same particle with tiny delta R veto
          if(( pow((p4_tag->Pt() - p4_probe->Pt()), 2) + pow((p4_tag->Eta() - p4_probe->Eta()), 2) < .000001) ) continue;

          bool pass_trigger_soup=false;
          // Loop over the trigger soup to see if this pair passes any of those triggers
          // First check if either of the leptons pass the single lepton triggers
          for(unsigned int it=0; it<soup_triggers_1l_.size() && !pass_trigger_soup; it++) {
            int soup_trigger;
            std::istringstream(soup_triggers_1l_[it]) >> soup_trigger;
            if( ((*triggerLeps)[ index_soup_tag_[iTag] ] & (0x1 << soup_trigger)) != 0 ||
                ((*triggerLeps)[ index_soup_probe_candidate_[iProbe] ] & (0x1 << soup_trigger)) != 0
            ) pass_trigger_soup=true;
          }
          // If that didn't fly, check if the lepton pair passed any of the dilepton triggers
          for(unsigned int it=0; it<soup_triggers_2l_.size() && !pass_trigger_soup; it++) {
            int soup_trigger;
            std::istringstream(soup_triggers_2l_[it]) >> soup_trigger;
            if( ((*triggerLeps)[ index_soup_tag_[iTag] ] & (0x1 << soup_trigger)) != 0 &&
                ((*triggerLeps)[ index_soup_probe_candidate_[iProbe] ] & (0x1 << soup_trigger)) != 0
...
///////////////////////////////////////////////////////////////////////
/*void make_tracking_skim(
  //string list_of_files,
  string input_file_name,
  string output_basename,
  string flavor="electron",
  trigger=0,
